...
const LoginPage = () => {
	...
	const {authorized, error} = useAppSelector(state => state.userSliceReducer);
				...
				<div className={styles.fieldsWrapper}>
					<label className={styles.formLabel} htmlFor="username">Username
						<p className={styles.formError}>{errors?.username &&
							<span>{(errors?.username?.message as string) || "Your username length should be from 4 to 20 symbols"}</span>}
						</p>
					</label>
					<input
					...
					<label className={styles.formLabel} htmlFor="password">Password
						<p className={styles.formError}>{errors?.password &&
							<span>{(errors?.password?.message as string) || "Your password min length should be 6 symbols"}</span>}</p>
					</label>
					<input
